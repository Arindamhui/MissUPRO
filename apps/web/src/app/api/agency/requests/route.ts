...
export async function GET(request: Request) {
  ...
  try {
    ...
    const items = await agencyService.listRequests({ userId: claims.sub, publicId: claims.publicId ?? null, email: claims.email, role: claims.role, sessionId: claims.sid, agencyId: claims.agencyId ?? null }, query.limit + 1, offset);
