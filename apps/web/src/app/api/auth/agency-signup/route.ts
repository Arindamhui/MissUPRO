...
import { db, users, admins, agencies, agencyHosts } from "@missu/db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    let [existingAgency] = await db.select().from(agencies).where(
    ...
    if (!existingAgency) {
      ...
      const publicId = generateCode("AGY", 8);
    ...
    if (!existingAgency) {
      return NextResponse.json({ status: "access_denied", reason: "agency_record_missing", role: null, platformRole: "AGENCY", userId: appUser.id, email, sessionId } satisfies PortalSession);
    }
