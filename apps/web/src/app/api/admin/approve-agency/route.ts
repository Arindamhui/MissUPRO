...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await withIdempotency({
      ...
      payload: input,
      execute: () => adminService.approveAgency({ userId: claims.sub, publicId: claims.publicId ?? null, email: claims.email, role: claims.role, platformRole: claims.platformRole, agencyStatus: claims.agencyStatus, sessionId: claims.sid, agencyId: claims.agencyId ?? null }, input, context),
    });
